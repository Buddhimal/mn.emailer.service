...
class communicator_email_queue {
	...
	function get_new_items_queued(){
		...
		try {
			...
			$sql = sprintf("SELECT id,sender_name,sender_email,email_type_id,send_to,template_id,content,delivery_status,delivered, error,is_deleted,is_active,updated,created,updated_by,created_by FROM communicator_email_queue WHERE delivery_status = %d", EmailStatus::Pending);
